...
| AEO-01 | **"Definição em caixa" + respostas em 40-60 palavras** nos artigos | Cap. 6.7 — táticas AEO para Featured Snippets. | Campo `summary` (40–60 palavras) nos **11 posts** + box "Em resumo" renderizado no topo do artigo. | 🟢 P2 | M | [x] |
...
| ID | Tarefa | Por quê (ref. guia) | Como / Critério de aceite | Prio | Esf | Status |
|----|--------|---------------------|---------------------------|------|-----|--------|
| PERF-01 | **Medir Core Web Vitals reais** (Lighthouse/PSI) | Cap. 6.5 / 8.2 — metas LCP < 0,8s, INP < 200ms, CLS < 0,05; "Lighthouse > 95". | Lighthouse prod local (mobile): **perf 53→89** após otimizar CrossParticles (≤30fps, pausa offscreen, menos partículas → **TBT 9050→20ms**) e hero `eager` (não bloqueia LCP). CLS 0, FCP 0,9s, SI 1,6s. LCP ~3,8s **instável localmente** (máquina sob carga; elemento LCP não capturado) → medir na infra Vercel/PSI. | 🔴 P0 | M | [~] |
